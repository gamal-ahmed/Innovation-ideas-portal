...
public class UserController implements Serializable {
	...
	public String addNewUser() {
		...
		if (userDAO.getUserByEmail(currentUser.getEmail()) != null) {
			...
			// currentUser.setDiscussionThread(new
			// DiscussionThread(currentUser));
			// }
	...
	private String getChangePasswordURL(String email) throws Exception {
		...
		HttpServletRequest request = (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest();
		String path = SettingCashManager.getIistance().getSettingValue(SettingsKeys.CONTEST_URL)+ request.getContextPath() + "/changePassword.xhtml" + "?token1="
				+ URLUtility.URLEncoding(encriptedmail) + "&token2=" + URLUtility.URLEncoding(encryptedDate);
		return path;
	}
