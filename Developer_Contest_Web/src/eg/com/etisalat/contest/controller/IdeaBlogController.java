...
public class IdeaBlogController {
	...
	public int getUserVote() {
		...
		if (userSessionController.getCurrentUser() != null) {
			userVote = voteDAO.getVoteByUserAndBlog(userSessionController.getCurrentUser(), currentIdeaBlog);
		}
		if (userVote != null)
			userRating = userVote.getVoteValue();
	...
	public void initiateBlog(ComponentSystemEvent systemEvent) {
		...
		// the current idea should be retrieved from the currentIdeaBlog
		if (currentIdea == null) {
	...
	public void loadBlog(ComponentSystemEvent systemEvent) {
		...
		System.out.println("loadBlog --- started");
		if (currentIdeaBlog == null) {
			currentIdeaBlog = (IdeaBlog) getFromFlash("blog");
			if (currentIdeaBlog == null) {
		...
		System.out.println("loadBlog --- ended");
	}
	...
	public void onrate() {
		String vote = ((HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest()).getParameter("star");
		int star = 0;
		if (vote != null)
			star = Integer.parseInt(vote);
		currentVote = null;
		initiateVote();

		currentVote.setVoteValue(star);
		ideaBlogDao.increasePositiveRate(currentIdeaBlog, currentVote, userSessionController.getCurrentUser());

		// currentVote.setVoteValue(rating);
		// if (ideaBlogDao.increasePositiveRate(currentIdeaBlog, rating)) {
		// voteDAO.addEntity(currentVote);
		// currentIdeaBlog.setPositiveRating(currentIdeaBlog.getPositiveRating()
		// + rating);
		// }
		// } else if (currentVote.getVoteValue() == 1) {
		//
		...
		// voteDAO.updateEntity(currentVote);
		// currentIdeaBlog.setNegativeRating(currentIdeaBlog.getNegativeRating()
		// - 1);
		// currentIdeaBlog.setPositiveRating(currentIdeaBlog.getPositiveRating()
		// + 1);
		// }
